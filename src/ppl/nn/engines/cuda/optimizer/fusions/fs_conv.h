    ...
    static bool CanFuseRelu(ir::Node* nextnode, const OptKernelOptions& options) {
        ...
        if (relu_fuse_op.find(nextnode->GetType().name) != relu_fuse_op.end()) {
            if (nextnode->GetType().name == "PRelu") { // extra check for PRelu
                ...
                const ppl::common::TensorShape& shape1 = *options.tensors->find(nextnode->GetInput(0))->second->GetShape();
                const ppl::common::TensorShape& shape2 = *options.tensors->find(nextnode->GetInput(1))->second->GetShape();
